...
// ═══════════════════════════════════════════════════════════════
// ZEHNX ACADEMY BOT — Studienberater & Lernpfad-Planer
...
const ACADEMY_BOT_SYSTEM = `Du bist ACADEMY BOT — der persoenliche Studienberater der ZEHNX Academy. Du bist KEIN allgemeiner Chatbot. Du hast EINE Mission: In 2-4 Nachrichten den perfekten Lernpfad zusammenstellen.
...
// ═══ ACADEMY BOT COMPONENT ═══

export default function AcademyBot({ onNavigate }) {
  ...
  if (!isOpen) {
    return (
      <button onClick={() => setIsOpen(true)} aria-label="ACADEMY BOT oeffnen" style={{
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: "-0.02em" }}>ACADEMY BOT</div>
        ...
        {messages.length === 0 && (
          <div style={{ padding: "4px 0" }}>
            <div style={{
              ...
              Hey! Ich bin der ACADEMY BOT — dein persoenlicher Studienberater.{"\n"}Ich finde in 2-3 Fragen den perfekten Lernpfad fuer dich.
            </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", marginBottom: 10 }}>
            ...
            <div style={{ padding: "12px 16px", borderRadius: 16, borderBottomLeftRadius: 4, background: Z.borderLight }}>
              <span style={{ display: "flex", gap: 5 }}>
                {[0, 0.15, 0.3].map((d, i) => (
                  <span key={i} style={{ width: 7, height: 7, borderRadius: 4, background: Z.hint, animation: `academybotPulse 1s infinite ${d}s` }}/>
                ))}
              </span>
            </div>
          </div>
      ...
      <style>{`@keyframes academybotPulse{0%,100%{opacity:1}50%{opacity:.2}}`}</style>
    </div>
  );
}
